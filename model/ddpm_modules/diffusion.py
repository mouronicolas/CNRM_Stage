...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        loss_type='l1',
        conditional=True,
        #condition_type = classifier_free_guidance,
        schedule_opt=None
    ):
        super().__init__()
    ...
    def set_new_noise_schedule(self, schedule_opt, device):
        ...
        self.sampling_timesteps = schedule_opt['sampling_timesteps']
        
        self.sampling_eta = schedule_opt['sampling_eta']
    ...
    @torch.no_grad()  
    def ddim_sample(self, x_in, condition_x =None,continuous = False):
        device = self.betas.device
        ...
        if not self.conditional:
            ...
            for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
                ...
                pred_noise = self.denoise_fn(img,time_cond) #denoise_fn(torch.cat([condition_x, x], dim=1), t)) denoise_fn(x, t))
            ...
            for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
                time_cond = torch.full((b,), time, device = device, dtype = torch.long)
                pred_noise = self.denoise_fn(torch.cat([x,img],dim=1),time_cond)
                x_start = self.predict_start_from_noise(img, t=time_cond, noise = pred_noise)
    ...
    def sample(self, batch_size=1, continous=False):
        ...
        if sampling_timesteps < total_timesteps :
            return self.ddim_sample((batch_size, channels, image_size, image_size), continous)
        else :
